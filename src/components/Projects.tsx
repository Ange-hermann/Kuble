...
const projects: {
...
}[] = [
  {
    ...
    color: '#00C2FF',
    emoji: '🏛️',
    bg: 'linear-gradient(135deg, #060D2E 0%, #0A1650 100%)',
    image: '/mairie.jpeg',
    link: 'https://mamairieci.netlify.app/',
  },
  {
    id: 2, categories: ['Web', 'Mobile'],
    title: 'Eglise CTF',
    desc: 'Plateforme web & mobile de gestion pour eglise : membres, evenements, dimes, communication.',
    stack: ['Web', 'Mobile', 'Gestion'],
    color: '#5BC8F2',
    emoji: '⛪',
    bg: 'linear-gradient(135deg, #0A1650 0%, #0D1B4C 100%)',
    image: '/app.png',
    link: 'https://eglisectf.org/',
  },
];
...
export default function Projects() {
  ...
  return (
    <section id="projets" style={{ padding: '6rem 2rem', background: 'linear-gradient(180deg, #E8F0FE 0%, #F0F5FF 100%)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        >
          ...
          <h2 style={{ fontFamily: FONTS.display, fontWeight: 800, fontSize: 'clamp(2rem, 4vw, 3rem)', color: COLORS.navyDeep, letterSpacing: '-0.02em' }}>Nos Réalisations</h2>
          ...
          {filters.map((f) => (
            <motion.button
              ...
              style={{
                ...
                borderRadius: 100,
                fontFamily: FONTS.display, fontWeight: 600, fontSize: '0.85rem',
                cursor: 'pointer',
                transition: 'all 0.3s ease',
                border: active === f ? 'none' : '1px solid rgba(10,22,80,0.15)',
                background: active === f ? `linear-gradient(135deg, ${COLORS.electric}, ${COLORS.cyan})` : 'transparent',
                color: active === f ? '#fff' : 'rgba(10,22,80,0.6)',
                boxShadow: active === f ? '0 0 16px rgba(91,200,242,0.4)' : 'none',
              }}
            >
              {f}
            </motion.button>
        ...
        {/* Grid */}
        <motion.div layout style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(280px, 100%), 1fr))', gap: '1.5rem' }}>
            ...
            {filtered.map((project) => (
              <motion.div
                ...
                style={{
                  borderRadius: 14,
                  overflow: 'hidden',
                  border: '1px solid rgba(10,22,80,0.08)',
              ...
              >
                ...
                <div style={{
                  ...
                  boxShadow: '0 4px 20px rgba(10,22,80,0.04)',
                }}>
                  <h3 style={{ fontFamily: FONTS.display, fontWeight: 700, fontSize: '1.05rem', color: COLORS.navyDeep, marginBottom: 6 }}>
                    {project.title}
                  </h3>
                  <p style={{ fontFamily: FONTS.body, fontSize: '0.85rem', color: 'rgba(10,22,80,0.6)', marginBottom: 12, lineHeight: 1.5 }}>
                    {project.desc}
                  </p>
                ...
                <div className="project-overlay" style={{
                  position: 'absolute', inset: 0,
                  background: `linear-gradient(135deg, ${project.color}dd, rgba(91,200,242,0.9))`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
